...
def footprint_commune(bbox1, bbox2):
...
def set_projection(chemin, dtype):
        ...
        for j in range(len(liste_n) - 1):
            ...
            for layer in liste_layers:
                if not tuile_trouvee:
                    #On vérifie que la couche possède une footprintcommune avec la dalle
            ...
            if not tuile_trouvee:
                ...
                try:
                    ...
                    write_tfw(os.path.join(path_meta_ortho, 'ORTHO_dalle_{}_{}.tfw'.format(i, j)), e_min_dalle, n_max_dalle, resolution)
                    set_projection(chemin, gdal.GDT_Byte)
                    #On télécharge le MNS
...
def filter_layers(liste_layers, bbox):
    #On ne conserve que les couches qui ont une footprintcommune avec le chantier
    dict_filtre = {}
    for layer in liste_layers:
        bbox_layer = liste_layers[layer]
        if footprint_commune(bbox_layer, bbox):
...
#Sur le geoserver pompei où se trouvent les orthos historiques déjà produites
adresse_geoserver = "" # A completer avec l'adresse de votre geoserver


#Charge l'footprintdu chantier
