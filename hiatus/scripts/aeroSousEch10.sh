...
if test -f ../Ortho-MEC-Malt-Abs-Ratafia/Orthophotomosaic_Tile_0_0.tif; then
    ${scripts_dir}/HIATUS.LINUX Reproj resultpt bidon bidon Orthophotomosaic_Tile_0_0.ori Orthophotomosaic_Tile_0_0.ori resultpi
    ${scripts_dir}/RANSAC resultpi resultpif --taille_case 5000 > /dev/null
    #On reprojette en terrain
    ${scripts_dir}/HIATUS.LINUX ReprojTerrain resultpif Orthophotomosaic_Tile_0_0.ori Orthophotomosaic_Tile_0_0.ori resultpifreproj
else
    ${scripts_dir}/HIATUS.LINUX Reproj resultpt bidon bidon Orthophotomosaic_Tile.ori Orthophotomosaic_Tile.ori resultpi
    ${scripts_dir}/RANSAC resultpi resultpif --taille_case 5000 > /dev/null
    #On reprojette en terrain
    ${scripts_dir}/HIATUS.LINUX ReprojTerrain resultpif Orthophotomosaic_Tile.ori Orthophotomosaic_Tile.ori resultpifreproj
fi
...
${scripts_dir}/HIATUS.LINUX AssocierZ_fichierpts2D:multiMNS pts_bdortho.txt liste_mns_bdortho.txt pts3D_bdortho.txt ; 
${scripts_dir}/HIATUS.LINUX AssocierZ_fichierpts2D:multiMNS pts_orthomicmac_abs.txt liste_mnsmicmac.txt pts3D_orthomicmac_abs.txt


#On recharge les deux fichiers 3D pour nettoyage des valeurs aberrantes (nodata = 9999) et preparation de letape suivante.
${scripts_dir}/HIATUS.LINUX BilanPts3D pts3D_bdortho.txt pts3D_orthomicmac_abs.txt pts3D_bdortho_net.txt pts3D_orthomicmac_abs_net.txt
...
${scripts_dir}/HIATUS.LINUX BilanApp2MICMAC pts3D_bdortho_net.txt liste_cliches.txt ../GCP-S2D.xml ../GCP-S3D.xml ../GCP.xml ../id_GCP.txt 
cd ..
