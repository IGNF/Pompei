...
python ${scripts_dir}/hiatus_rapide/generate_ori.py --metadata metadata --ta_xml ${TA}
...
for i in `cat liste_dalles.txt` ; do
    ...
    echo -n "${scripts_dir}/HIATUS.LINUX DecalageDalle:CropResult dallage/${i}.ori ../appuisSousEch10/resultpi dallage/${i}.pregeoref 5000 ; " >> bashtmp ;
    ...
    echo "${scripts_dir}/HIATUS.LINUX DecalageDalle:FromSim dallage/${fichiersansext}.ori dallage/${fichiersansext}.pregeoref_modsim > dallage/${i}.T.txt ; " >> bashtmp ;
done
...
cd ..
python ${scripts_dir}/hiatus_rapide/create_one_ortho_per_image.py --metadata metadata --ta_xml ${TA} --factor 1 --workdir appuis  --decalage True
...
for i in `cat liste_dalles.txt` ; do 
    ...
    echo "${scripts_dir}/HIATUS.LINUX NettoyageResultZoneNoire  dallage/${i}.resultpi dallage/${i}.tif dallage/${i}.resultpi > /dev/null" >> bashtmp1 
done
...
python ${scripts_dir}/hiatus_rapide/reproj_appuis.py --factor 1 --regroupe True
...
cd ..
python ${scripts_dir}/hiatus_rapide/Aubry_convert_GCP_to_mm3d.py --metadata metadata