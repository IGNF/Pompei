...
def createBuffer(inputfn, outputBufferfn, bufferDist):
    #Applique un buffer sur les emprises
...
def merge(src, dst, typeData):
    #Regroupe toutes les emprises qui sont connexes
    ...
    if id > 1:
        if typeData=="buffer":
            print("Attention : des images sont isolées sur le chantier. Regardez les fichiers {} et {}". format(path_emprise_chantier, path_buffer))
        elif typeData=="recouvrement":
            print("Attention : le recouvrement n'est pas idéal. Regardez les fichiers {} et {}". format(path_emprise_chantier, path_buffer))


def recouvrement(chemin_emprise, chemin_resultat):
    schema = {                                                                                                     
        "geometry": "Polygon",                                                                                     
    }
    emprises1 = fiona.open(chemin_emprise)
    with fiona.open(chemin_resultat, 'w',driver='ESRI Shapefile', schema=schema, crs=emprises1.crs) as output:
        for emp1 in emprises1:
            for emp2 in fiona.open(chemin_emprise):
...
def calcul_proportion_recouvrement(path_recouvrement_merged, path_emprise_chantier_merged):
    aire_recouvrement = 0
    aire_chantier = 0
    for emp1 in fiona.open(path_recouvrement_merged):
        aire_recouvrement += shape(emp1['geometry']).area
    for emp2 in fiona.open(path_emprise_chantier_merged):
...
def get_nb_couleurs():
    ...
    with open(os.path.join(chemin_metadata, "nbCouleurs.txt"), "w") as f:
        f.write(str(nbBands))


os.makedirs(os.path.join(chemin_chantier, "Analyse_Plan_Vol"), exist_ok=True)
path_emprise_chantier = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "chantier.shp")
path_emprise_chantier_merged = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "chantier_merged.shp")
path_buffer = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "buffer.shp")
path_merged = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "buffer_merged.shp")
path_recouvrement = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "recouvrement.shp")
path_recouvrement_merged = os.path.join(chemin_chantier, "Analyse_Plan_Vol", "recouvrement_merged.shp")
...
save_shapefile(liste_images, images, path_emprise_chantier, EPSG)
...
createBuffer(path_emprise_chantier, path_buffer, distance_buffer)

#Regroupe les emprises qui se superposent. Si l'on obtient au moins deux features, alors on met un message d'alerte : il est probable que des images soient déconnectées du reste du chantier
merge(path_buffer, path_merged, "buffer")

#Récupère les zones du chantier qui sont vues par au moins deux photos
recouvrement(path_emprise_chantier, path_recouvrement)
...
merge(path_emprise_chantier, path_emprise_chantier_merged, "emprises")

calcul_proportion_recouvrement(path_recouvrement_merged, path_emprise_chantier_merged)
