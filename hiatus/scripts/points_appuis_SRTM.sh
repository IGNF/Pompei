...
python ${repertoire_scripts}/download_SRTM.py --MNS_Histo MEC-Malt-Abs-Ratafia/MNS_Final_Num8_DeZoom2_STD-MALT.tif --metadata metadata --output metadata/mns_temp/mns.tif
...
python ${repertoire_scripts}/resample.py --input_SRTM metadata/mns_temp/decoupe_byte.tif --output_SRTM MEC_SRTM/DSM_SRTM.tif --res 2 --input_histo MEC-Malt-Abs-Ratafia/DSM_histo-gray.tif --output_histo MEC-Malt-Abs-Ratafia/DSM_histo-gray.tif 
...
mkdir TraitementAPP
python ${repertoire_scripts}/get_Appuis_SRTM.py --input_points Tmp_Patches-CoReg/Homol-SubPatch-2DRANSAC/PastisMEC_SRTM.tif/MEC-Malt-Abs-Ratafia.tif.txt --output_points TraitementAPP/resultpi --input_image metadata/mns_temp/decoupe_byte.tif
...
if test -f ../Ortho-MEC-Malt-Abs-Ratafia/Orthophotomosaic_Tile_0_0.tif; then
    ${repertoire_scripts}/HIATUS.LINUX Reproj resultpt bidon bidon Orthophotomosaic_Tile_0_0.ori Orthophotomosaic_Tile_0_0.ori resultpi >> logfile
    ${repertoire_scripts}/RANSAC resultpi resultpif --taille_case 5000 > /dev/null >> logfile
    #On reprojette en terrain
    ${repertoire_scripts}/HIATUS.LINUX ReprojTerrain resultpif Orthophotomosaic_Tile_0_0.ori Orthophotomosaic_Tile_0_0.ori resultpifreproj >> logfile
else
    ${repertoire_scripts}/HIATUS.LINUX Reproj resultpt bidon bidon Orthophotomosaic_Tile.ori Orthophotomosaic_Tile.ori resultpi >> logfile
    ${repertoire_scripts}/RANSAC resultpi resultpif --taille_case 5000 > /dev/null >> logfile
    #On reprojette en terrain
    ${repertoire_scripts}/HIATUS.LINUX ReprojTerrain resultpif Orthophotomosaic_Tile.ori Orthophotomosaic_Tile.ori resultpifreproj >> logfile
fi