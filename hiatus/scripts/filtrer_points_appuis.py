...
def createBufferBati(path_dalles_json, outputBufferfn, bufferDist):
    """
...
def download_data(bbox):
    ...
    path_bati = createBufferBati(path_tuile_bati, "BD_Topo_buffer.shp", 20)
    ...
    path_vegetation = createBufferBati(path_tuile_vegetation, "BD_Topo_buffer.shp", 0)
    ...
    #On applique un buffer qui ici ne fait que fusionner les différentes dalles
    path_hydro = createBufferBati(path_tuile_hydro, "BD_Topo_buffer.shp", 0)
    
    return path_bati, path_vegetation, path_hydro 


def traitement_points_appuis(path_bati, path_vegetation, path_hydro):
    ...
    with open(args.appuis_save, "w") as f:
...
def delete_appuis(liste_appuis_supprimes):
...
def save_appuis_id(liste_points_conserves):
    with open("id_appuis.txt", "w") as f:
...
liste_appuis_supprimes, liste_points_conserves = traitement_points_appuis(path_bati, path_vegetation, path_hydro)

delete_appuis(liste_appuis_supprimes)

save_appuis_id(liste_points_conserves)
