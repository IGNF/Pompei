...
python ${repertoire_scripts}/delete_black_images.py --path dallage --seuil_value 0 --seuil_prop 0.98
...
echo -n "" > bashtmp
for i in `cat liste_dalles.txt` ; do 
#On cree une dalle d ortho superposable en tenant compte du décalage calculé dans points_appuis_sous_echantillonnage_10.sh
echo -n "${repertoire_scripts}/HIATUS.LINUX DecalageDalle:CropResult dallage/${i}.ori ../TraitementAPPssech10/resultpi dallage/${i}.pregeoref 5000 ; " >> bashtmp ;
...
echo -n "${repertoire_scripts}/HIATUS.LINUX DecalageDalle:FromSim dallage/${i}.ori dallage/${i}.pregeoref_modsim > dallage/${i}.T.txt ; " >> bashtmp ;
echo -n "${repertoire_scripts}/HIATUS.LINUX TranslatOri:m dallage/${i}.ori dallage/${i}.T.txt dallage/${i}.decal.ori ; " >> bashtmp ;
